...
import { RedressMechanismPanel } from "@/components/arta/redress-mechanism/RedressMechanismPanel";
...
export default function Header() {
  ...
  const openRedress = () => setIsRedressOpen(true);
      ...
      createPortal(
        <div
          className={`fixed inset-0 z-[9999] transition-opacity duration-300 ease-in-out ${
            isRedressOpen
              ? "opacity-100 pointer-events-auto bg-black/40 backdrop-blur-xs"
              : "opacity-0 pointer-events-none"
          }`}
          onClick={closeRedress}
          aria-hidden={!isRedressOpen}
        >
          <div
            ref={redressModalRef}
            className={`absolute inset-y-0 right-0 h-full w-full sm:w-[480px] transition-transform duration-300 ease-in-out ${
              isRedressOpen ? "translate-x-0" : "translate-x-full"
            }`}
